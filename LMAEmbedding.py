...
class LMAEmbeddingFunction(torch.autograd.Function):
    ...
    def forward(ctx, hashed_weights, input_embeddings, lsh_matrix, random_numbers, input_dim, embedding_dim, num_chunks, chunk_size, bits_per_chunk, lsh_mode):
        '''
        ...
        output, hashed_idx = \
            lma_embedding_bag.forward(hashed_weights, input_embeddings, lsh_matrix, random_numbers,  input_dim, embedding_dim, num_chunks, chunk_size, bits_per_chunk, lsh_mode_enum)
    ...
    def backward(ctx, grad):
        ...
        return weight_grad, None, None, None, None, None, None, None, None
...
class LMAEmbedding(nn.Module):
    def __init__(
        ...
        if (lsh_mode == "srp"):
            self.lsh_matrix = Parameter(torch.from_numpy(generateSparseSRP(input_dim, self.num_chunks * self.bits_per_chunk).astype(np.float32)), requires_grad=False)
        else:
        ...
        print("LMAEmbedding: input_dim: ", input_dim, " embedding_dim:", embedding_dim, " chunk_size:",
              chunk_size, " bits_per_chunk:", bits_per_chunk, "lsh_mode:", lsh_mode, " seed:", seed, " lsh matrix", self.lsh_matrix.shape)
    ...
    def forward(self, input_embeddings) -> torch.Tensor:
        ...
        embeddings =  LMAEmbeddingFunction.apply(
            self.weight, input_embeddings, self.lsh_matrix, self.random_numbers,  self.input_dim, self.embedding_dim, self.num_chunks, self.chunk_size, self.bits_per_chunk, self.lsh_mode
        )
        return embeddings
